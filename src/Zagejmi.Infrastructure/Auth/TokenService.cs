...
public class TokenService : ITokenService
{
    ...
    public string GenerateToken(Person person)
    {
        ...
        if (person.PersonalInformation.MailAddress?.Address != null)
        {
            var claims = new[]
            {
                new Claim(JwtRegisteredClaimNames.Sub, person.Id.ToString()),
                new Claim(JwtRegisteredClaimNames.Email, person.PersonalInformation.MailAddress?.Address ?? string.Empty),
